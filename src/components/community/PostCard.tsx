...
function AnonAvatar({ seed, handle, size = 36 }: { seed: string; handle: string; size?: number }) {
  ...
  return (
    ...
    >
      <Body
        className="font-body-bold text-chalk"
    ...
    <View className="flex-row gap-2.5">
      ...
      <View className="flex-1">
        <View className="flex-row items-center gap-2">
          <Label className="text-chalk">{comment.handle}</Label>
          <Body className="text-xs text-ash">{comment.timeLabel}</Body>
          <View className="flex-1" />
          {!comment.isMine && (
            <Pressable hitSlop={8} onPress={confirmReport} accessibilityRole="button">
              <Flag size={14} color={colors.ash} />
            </Pressable>
          )}
        </View>
        <Body
          className={`mt-1 text-sm leading-5 ${isPending ? 'text-chalk/50' : 'text-chalk'}`}
        >
          {comment.body}
        </Body>
...
export function PostCard({
  ...
  return (
    <RiseIn index={index}>
      <View className="rounded-2xl border border-line bg-coal px-4 py-4">
        {/* Identity row — handle / avatarSeed / timeLabel and NOTHING else */}
        <View className="flex-row items-center gap-3">
          <AnonAvatar seed={item.avatarSeed} handle={item.handle} />
          <View className="flex-1">
            <View className="flex-row items-center gap-2">
              <Label className="text-chalk">{item.handle}</Label>
              {item.isMine && <Label className="text-volt">you</Label>}
            </View>
            <Body className="text-xs text-ash">{item.timeLabel}</Body>
          </View>
          {groupTagName && <Pill>{groupTagName}</Pill>}
        </View>

        {/* Body — your own pending post sits dimmed with no-shame copy */}
        <Body
          className={`mt-3 text-[15px] leading-6 ${isPending ? 'text-chalk/50' : 'text-chalk'}`}
        >
          {item.body}
        </Body>
        ...
        <View className="mt-3 flex-row items-center gap-5">
          ...
          >
            <Heart
              size={18}
              strokeWidth={2.5}
              color={item.myReaction ? colors.volt : colors.ash}
              fill={item.myReaction ? colors.volt : 'transparent'}
            />
            <Body
              className={`font-body-semibold text-xs ${item.myReaction ? 'text-volt' : 'text-ash'}`}
            >
              {REACTION_LABEL}
          ...
          >
            <MessageCircle
              size={18}
              strokeWidth={2.5}
              color={commentsOpen ? colors.chalk : colors.ash}
            />
            {item.commentCount > 0 && (
              <Body
                className={`font-body-semibold text-xs ${commentsOpen ? 'text-chalk' : 'text-ash'}`}
              >
                {item.commentCount}
              </Body>
            )}
          ...
          <SpringPressable hitSlop={8} onPress={openOverflow} accessibilityRole="button">
            <MoreHorizontal size={18} color={colors.ash} />
          </SpringPressable>
        </View>
