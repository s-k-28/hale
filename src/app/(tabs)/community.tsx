        ...
        {globalGroup ? (
          <RiseIn index={0}>
            <Pressable
              ...
              className="mb-3 rounded-2xl border border-volt/30 bg-volt/10 px-5 py-4 active:opacity-80"
            >
              <View className="flex-row items-center gap-3">
                <Globe color={colors.volt} size={22} strokeWidth={2.5} />
                <View className="flex-1">
                  <Heading className="text-base">{globalGroup.name}</Heading>
                  <Body className="mt-1 font-body text-xs leading-4 text-ash">
                    {COMMUNITY_GROUPS[globalGroup.slug]?.description}
                  </Body>
                </View>
                <ChevronRight color={colors.volt} size={20} strokeWidth={2.5} />
              </View>
              <View className="mt-3 flex-row items-center gap-1.5">
                <Users color={colors.ash} size={13} strokeWidth={2.5} />
...
function GroupCard({
  ...
  return (
    <Pressable
      ...
      className="mb-3 rounded-2xl border border-line bg-coal px-5 py-4 active:opacity-80"
    >
      ...
      <View className="mt-3 flex-row items-center justify-between">
        <View className="flex-row items-center gap-1.5">
          <Users color={colors.ash} size={13} strokeWidth={2.5} />
          <Body className="font-body text-xs text-ash">{memberLabel(group.memberCount)}</Body>
        </View>
        ...
        {group.joined && myProfile ? (
          ...
          <View className="flex-row items-center gap-2">
            ...
            >
              <Body className="font-body-bold text-[10px] text-white">
                {myProfile.handle.charAt(0).toUpperCase()}
              </Body>
            </View>
            <Body className="font-body-medium text-xs text-ash">
              Joined as <Body className="font-body-semibold text-xs text-chalk">{myProfile.handle}</Body>
            </Body>
          </View>
        ) : (
          <Pressable
            ...
            className="rounded-full bg-volt px-4 py-2 active:opacity-90"
          >
            {joining ? (
              <ActivityIndicator size="small" color={colors.voltInk} />
            ) : (
              <Heading className="text-xs text-volt-ink">Join</Heading>
            )}
          </Pressable>
        )}
      </View>
